...
/**
 * Database meta-data layer for HSQLDB.
 *
 * @author Copyright (c) Alfa Financial Software 2020
 */
class HSqlDBMetaDataProvider extends DatabaseMetaDataProvider {

    /**
   * @param connection DataSource to provide meta data for.
   */
  public HSqlDBMetaDataProvider(Connection connection) {
  ...
  protected boolean isIgnoredTable(RealName tableName) {
    ...
    return tableName.getDbName().startsWith(HSqlDBDialect.TEMPORARY_TABLE_PREFIX);
  }
